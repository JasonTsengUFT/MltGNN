...
parser.add_argument('--time_inter', type=int, default=5, help='Time inter for predict')
...
train_input, test_input, train_target, test_target = train_test_split(x_train, y_train, test_size=20, shuffle=args.shuffle, random_state=42)

# y_train = pd.DataFrame(scaler.inverse_transform(y_train), columns=print_columns)
...
def train(batch_x, batch_y, model):
    ...
    loss_train = criterion(output_grad, target)

    # print(loss_train)
    # sys.exit()
    results = torch.squeeze(torch.cat([output_grad, target],dim=0))
    error = np.sqrt(MSE(output_grad.detach().numpy(),target.detach().numpy()))
    loss_train.backward()
    optimizer.step()
...
def train_model(model):
    ...
    for epoch in range(args.epochs):
        ...
        for stept, (batch_xt, batch_yt) in enumerate(seldata_loader):
            ...
            acct = acc_f(output_gradt.detach(), targett.detach())

            RMSEValue_epoch.append(errort)
            Acc_epoch.append(acct)
    ...
    for stept, (batch_xte, batch_yte) in enumerate(test_loader):
        ...
        accte = acc_f(output_gradte.detach(), targette.detach())
        RMSEValue_e.append(errort)
        Acc_e.append(acct)
